...
void MainWindow::StartRace(void){
	...
	if(ui->rb_qtConcur->isChecked()){
		...
		auto task1 = [&](){concurRace1->DoWork(&number, false, ui->sb_initNum->value());};
		auto task2 = [&](){concurRace2->DoWork(&number, false, ui->sb_initNum->value());};
		QtConcurrent::run(task1).then(task2);

//		второй вариант
//		auto f1 = QtConcurrent::run(&ExampleRace::DoWork, concurRace1, &number, false, ui->sb_initNum->value());
//		f1.waitForFinished();
//		auto f2 = QtConcurrent::run(&ExampleRace::DoWork, concurRace2, &number, false, ui->sb_initNum->value());
    }
    else{
